...
class TestLoader():
    ...
    def __iter__(self):
        for i in range(self.len):
            seqs, labels = self.x[i][None, :], self.y[i][None, :]
            seq_lengths = torch.IntTensor(list(map(lambda x: x.shape[0], seqs)))
            max_seq_len = seq_lengths.max()

            # allocate spaces
            seq_padded = np.zeros((1, max_seq_len, self.channel), dtype=float)  # (n, max_len, channel)

            for i, (seq, seq_len) in enumerate(zip(seqs, seq_lengths)):
                seq_padded[i, :seq_len, :] = seq
